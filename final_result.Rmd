...
$$ Y_t =c+ϕ_1Y_{t−1}+ϕ_2Y_{t−2}+...+ϕ_pY_{t−p} +ε +θ_1ε_{t-1}+θ_2ε_{t-2} +...+θ_pε_{t-p}$$
Where 

1). $ϕ_1,ϕ_2...ϕ_p$ are the autoregressive coefficients.

2). $θ_1,θ_2...θ_p$ are the moving average coefficients.
...
```{r modelcb, fig.height =8, fig.cap="The historical trend + ETS model prediction in hot selling product"}
...
```{r rollavgcombo, fig.height=8, fig.cap ="The historical trend + 12 month rolling average prediction"}
...
- Using the average of demand forecasting for upcoming 6 months to prevent the volatility in Peak and trough season
...
## 🧭 Backend logic 
 ...
 <font color= #8b0000> Node 1: Full container analysis </font>
...
d). All conditions are unmatched : "Other strategy to wait for FCL consolidation"-- Rare case in actual practice
 ...
 <font color= #8b0000> Node 2: Cost analysis </font>
 
a). Prerequsite Condition: only consider for the sku cannot fulfill RDC but can consolidated to NDC

b). Regional cost:  container cbm* Sea freight + cbm*(inventory/storage + miscellaneous…)

c). NDC cost:  Consol cbm to RDC*(Sea freight + domestic transportation + inventory/storage + miscellaneous…)

d). NDC cost*(1 + cost  threshold)  -  Regional cost>= 0, revise NDC to RDC
...
```{r}
high_predict<-example_tsb_model_tbmt |> filter(`Item Code` %in% high & Model =="ETS")|> group_by(Region,`Item Code`)|> summarise(`Safety stock`=mean(`Safety stock`))

middle_ETS_predict<- example_tsb_model_tbmt |> filter(`Item Code` %in% middle & Region != "ADE"& Model =="ETS")|> group_by(Region,`Item Code`)|> summarise(`Safety stock`=mean(`Safety stock`))

low_predict<- example_mt_rbind|> filter(`Item Code` %in% low )|> group_by(Region,`Item Code`)|> summarise(`Safety stock`=mean(`Safety stock`))

middle_roll_predict<- example_mt_rbind |> filter(`Item Code` %in% middle & Region == "ADE")|> group_by(Region,`Item Code`)|> summarise(`Safety stock`=mean(`Safety stock`))

Other_predict<-   example_tsb_model_tbmt |> filter(!(`Item Code` %in% sample_list) & Model =="ETS")|> group_by(Region,`Item Code`)|> summarise(`Safety stock`=mean(`Safety stock`))
...
The table below documents the decision result summary for AMES testing products. 
We can notice that the structure of the result distribution is consistent across the regions. 
Only a few of the SKUs can solely fulfill the volume requirement for FCL(`1238`,`1271` and `1234` and `1264` in Sydney); the majority  still need cargo consolidation with other SKUs from the same supplier.Due to the remarkable cost difference in sea freight compared to domestic road freight charges, most of the RDC parcel decisions will be reassessed in the cost analysis node and  rerouted back to the RDC warehouse, which is consistent with the practical handling  scenarios observed in AMES. 


Brisbane and Adelaide are crucial sources for their  customers. Hence, it is reflected that all regional products will be directly bound to the RDC warehouse. And the sku in `2LI` still need to ship to NDC for consolidation first in Sydney and Adelaide.

Worth noting, in actual practice, there are rare scenarios for the roundup trade-off during single SKU, and the cubic meter (CBM) exists as a fault line between 80 CBM and 50 CBM.
